...
SeedBundle::SeedBundle(SBInstruction *SBI) : DmpVector<SBValue *>(SBI) {}

DmpVectorView<SBValue *> SeedBundle::getSlice(unsigned Offset,
  ...
  for (auto [ElementCnt, SBV] :
    ...
    if (isUsed(FirstGoodElementIdx + ElementCnt))
      break;
    BitsSum += SBVecUtils::getNumBits(SBV, DL);
    ...
    if (ForcePowOf2 && !SBVecUtils::isPowerOf2(BitsSum))
      continue;
  ...
  if (LastGoodBitsSum == 0)
    return {};
  DmpVectorView<SBValue *> Slice = DmpVectorView<SBValue *>(*this).slice(
      FirstGoodElementIdx, LastGoodNumSliceElements);
  assert((!ForcePowOf2 ||
          SBVecUtils::isPowerOf2(SBVecUtils::getNumBits(Slice, DL))) &&
         "Expected power of 2!");
...
MemSeedContainer::KeyT MemSeedContainer::getKey(LoadOrStoreT *LSI) const {
  ...
  Type *Ty = SBVecUtils::getElementType(SBVecUtils::getExpectedType(LSI));
...
bool MemSeedContainer::erase(SBValue *SBV) {
  assert((isa<SBLoadInstruction>(SBV) || isa<SBStoreInstruction>(SBV)) &&
         "Expected Load or Store!");
...
// Explicit instantiations
template void MemSeedContainer::insert<SBLoadInstruction>(SBLoadInstruction *);
template void
...
template <typename LoadOrStoreT> static bool isBadMemSeed(LoadOrStoreT *LSI) {
  if (!LSI->isSimple())
    return true;
  auto *Ty = SBVecUtils::getExpectedType(LSI);
  // Skip scalable vectors.
  if (isa<VectorType>(Ty) && !isa<FixedVectorType>(Ty))
    return true;
  Type *ElemTy = SBVecUtils::getElementType(Ty);
    ...
    if (LLVM_LIKELY(CollectInstrOps)) {
      if (SBI.getNumOperands() == 2) {
        ...
        if (OpLHS && OpRHS && OpLHS != OpRHS &&
          ...
          NonStoreSeeds.insert(DmpVector<SBValue *>{OpLHS, OpRHS});
      }
    }
  }
