; RUN: opt -aa-pipeline=basic-aa -passes='print<memoryssa>' -verify-memoryssa < %s 2>&1 | FileCheck %s --implicit-check-not=MemoryUse
;
; Things that BasicAA can prove points to constant memory should not have MemoryAccesses.
