...
define void @foo(ptr %ptr1, ptr %ptr2) {
  ...
  auto *S1 = &*It++;
  auto *Ret = &*It++;
  DmpVector<SBValue *> Vec({S0, S1, Ret});
  SBInstruction *TmpI = S0;
  for (SBInstruction *I : Vec.instrRange()) {
    EXPECT_EQ(I, TmpI);
...
define void @foo(ptr %ptr1, ptr %ptr2) {
  ...
  }
  {
    ...
    DmpVector<SBInstruction *> SBVec({S0, S1, Ret});
    DmpVector<SBInstruction *> SBVecCpy(SBVec);
    EXPECT_EQ(SBVec, SBVecCpy);
  }
  {
    // Check move constructor
    DmpVector<SBInstruction *> SBVec({S0, S1, Ret});
    DmpVector<SBInstruction *> SBVecMv(std::move(SBVec));
  ...
  {
    // Check reserve constructor
    DmpVector<SBInstruction *> SBVec(10);
  ...
  }
  {
    ...
    DmpVector<SBInstruction *> SBVec({S0, S1, Ret});
    DmpVector<SBInstruction *> SBVecCpy(SBVec.begin(), SBVec.end());
    EXPECT_EQ(SBVec, SBVecCpy);
  }
}
...
define void @foo(ptr %ptr1, ptr %ptr2) {
  ...
  {
    // Default constructor.
    DmpVectorView<SBInstruction *> SBVecView;
    EXPECT_TRUE(SBVecView.empty());
  }
  {
    // DmpVector consturctor.
    DmpVectorView<SBInstruction *> SBVecView(SBVec);
  ...
  {
    // Range constructor.
    DmpVectorView<SBInstruction *> SBVecView(SBVec.begin(),
                                             std::prev(SBVec.end()));
  ...
  }
  {
    ...
    ArrayRef<SBInstruction *> Array(SBVec.begin(), SBVec.end());
    DmpVectorView<SBInstruction *> SBVecView(Array);
  ...
  }
  {
    ...
    auto ImplConv = [](DmpVectorView<SBInstruction *> View) {};
    ImplConv(SBVec);
  }
  ...
  // be converted to DmpVectorView.
  {
    // slice(N, M)
    DmpVectorView<SBInstruction *> SBVecView(SBVec);
    DmpVectorView<SBInstruction *> SliceView = SBVecView.slice(1, 2);
  ...
  }
  {
    // slice(N)
    DmpVectorView<SBInstruction *> SBVecView(SBVec);
    DmpVectorView<SBInstruction *> SliceView = SBVecView.slice(1);
    EXPECT_EQ(SliceView.size(), 2u);
    EXPECT_EQ(SliceView[0], SBVec[1]);
    EXPECT_EQ(SliceView[1], SBVec[2]);
  }
  {
    // drop_back(N)
    DmpVectorView<SBInstruction *> SBVecView(SBVec);
    DmpVectorView<SBInstruction *> SliceView = SBVecView.drop_back(2);
