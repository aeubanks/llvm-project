...
unsigned InstructionMaps::getScalarLane(SBValue *Vec,
  ...
  for (auto It = Bndl.begin(); It != ScalarIt; ++It)
    CntLanes += SBVecUtils::getNumLanes(*It);
  return CntLanes;
}

const DmpVector<SBValue *> &InstructionMaps::getScalarsFor(SBValue *Vec) const {
  auto It = VectorToScalarsMap.find(Vec);
  static DmpVector<SBValue *> EmptyBndl;
...
InstructionMaps::getVectorsThatCombinedScalars(
    const DmpVector<SBValue *> &Bndl) const {
...
InstructionMaps::getShuffleMask(SBValue *Vec,
                                const DmpVector<SBValue *> &OtherBndl) const {
  const DmpVector<SBValue *> &Bndl = getScalarsFor(Vec);
  ...
  ShuffleIndices.reserve(SBVecUtils::getNumLanes(Vec));
  ...
  for (SBValue *OtherV : OtherBndl) {
    ...
    for (unsigned Cnt = 0, Lanes = SBVecUtils::getNumLanes(OtherV);
         Cnt != Lanes; ++Cnt)
