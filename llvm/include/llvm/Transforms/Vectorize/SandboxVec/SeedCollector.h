...
class SeedBundle : public DmpVector<SBValue *> {
  /// The lanes that we have already vectorized.
  BitVector UsedLanes;

public:
  explicit SeedBundle(DmpVector<SBValue *> &&Bndl) : DmpVector<SBValue *>(std::move(Bndl)) {}
  explicit SeedBundle(const DmpVector<SBValue *> &Bndl) : DmpVector<SBValue *>(Bndl) {}
  /// Initialize a seed with \p SBI.
  explicit SeedBundle(SBInstruction *SBI);
  /// No need to allow copies.
  SeedBundle(const SeedBundle &) = delete;
  SeedBundle &operator=(const SeedBundle &) = delete;
  virtual ~SeedBundle() {}
  using iterator = DmpVector<SBValue *>::iterator;
  using const_iterator = DmpVector<SBValue *>::const_iterator;
  SBInstruction *operator[](unsigned Idx) const {
    return cast<SBInstruction>(DmpVector<SBValue *>::operator[](Idx));
  }
  /// This will insert \p SBI into its sorting position.
  virtual bool tryInsert(SBInstruction *SBI, const DataLayout *DL,
                         ScalarEvolution *SE) {
  ...
  DmpVectorView<SBValue *> getSlice(unsigned Offset, unsigned MaxVecRegBits,
                                    bool ForcePowOf2,
...
public:
  explicit MemSeedBundle(DmpVector<SBValue *> &&Bndl) : SeedBundle(std::move(Bndl)) {
  ...
  bool tryInsert(SBInstruction *SBI, const DataLayout *DL,
                 ScalarEvolution *SE) final {
    assert(isa<LoadOrStoreT>(SBI) && "Expected a Store or a Load!");
    auto Cmp = [DL, SE](SBValue *SBV1, SBValue *SBV2) {
      return SBVecUtils::comesBeforeInMem(cast<LoadOrStoreT>(SBV1),
                                          cast<LoadOrStoreT>(SBV2), *SE, *DL);
    };
...
using StoreSeedBundle = MemSeedBundle<SBStoreInstruction>;
using LoadSeedBundle = MemSeedBundle<SBLoadInstruction>;
...
public:
  ...
  virtual bool erase(SBValue *SBV) = 0;
#ifndef NDEBUG
...
class SeedContainer : public SeedContainerBase {
  ...
  /// them.
  DenseMap<SBValue *, DenseSet<SeedBundle *>> SeedToBundlesMap;
...
public:
  ...
  void insert(DmpVector<SBValue *> &&Seeds) {
    auto SBPtr = std::make_unique<SeedBundle>(std::move(Seeds));
    auto *SB = SBPtr.get();
#ifndef NDEBUG
    DenseSet<SBValue *> Visited;
    Visited.reserve(SBPtr->size());
#endif
    for (SBValue *Seed : *SBPtr) {
...
class MemSeedContainer : public SeedContainerBase {
  using KeyT = std::tuple<Value *, Type *, SBInstruction::Opcode>;
  using ValT = SmallVector<std::unique_ptr<SeedBundle>>;
  using SeedMapT = MapVector<KeyT, ValT>;
  /// Map from {pointer value *, Type, Opcode} to a vector of bundles
  SeedMapT Seeds;
  ...
  DenseMap<SBValue *, SeedBundle *> LookupMap;
...
class SeedCollector {
  ...
  SBContext::RemoveCBTy *RemoveInstrCB;
  SBContext &Ctxt;

  /// Helper for collecting SB{Store,Load}Instruction seeds.
...
public:
  SeedCollector(SBBasicBlock *SBBB, const DataLayout &DL, ScalarEvolution &SE);
  ~SeedCollector();
