...
; RUN: llc --code-model=medium --large-data-threshold=20 < %s | FileCheck %s --check-prefix=CHECK-SMALL
; RUN: llc --code-model=medium < %s | FileCheck %s --check-prefix=CHECK-MEDIUM
; RUN: llc --code-model=kernel < %s | FileCheck %s --check-prefix=CHECK-KERNEL
