...
void X86_64Thunk::writeTo(uint8_t *buf) {
  ...
  buf[1] = 0x25;
  uint64_t s;
  if (GotPartitionSection *gp =
          getThunkTargetSym()->section->getOutputSection()->gotPartition)
    s = gp->addEntry(destination)->getVA(ctx);
  else
    s = destination.getGotVA(ctx);
  uint64_t p = getThunkTargetSym()->getVA(ctx);
...
void X86_64Thunk::addSymbols(ThunkSection &isec) {
  ...
  if (GotPartitionSection *gp = isec.getParent()->gotPartition) {
    gp->addEntry(destination);
  } else if (!destination.isInGot(ctx)) {
