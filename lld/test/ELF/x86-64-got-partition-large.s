...
# RUN: ld.lld --fatal-warnings -pie --got-partition-threshold=0x4000 -z max-page-size=4096 %t.o -o %t.exe -Map=%t.map
...
# DISASM-EMPTY:
# DISASM-NEXT: <_start>:
# DISASM-NEXT: {{.*}}movq{{.*}}(%rip), %rax{{.*}}# 0x3fe8
# DISASM:      <_start_2>:
# DISASM-NEXT: {{.*}}movq{{.*}}(%rip), %rax{{.*}}# 0x7b50
# DISASM:      <_start_3>:
# DISASM-NEXT: {{.*}}movq{{.*}}(%rip), %rax{{.*}}# 0xb6b8
# DISASM:      <_start_4>:
# DISASM-NEXT: {{.*}}movq{{.*}}(%rip), %rax{{.*}}# 0xb6b8
...
# PHDR:      GNU_RELRO      {{.*}} 0x0000000000003fe8 0x0000000000003fe8 0x000008 0x000008 R   0x1
# PHDR-NEXT: GNU_RELRO      {{.*}} 0x0000000000007b50 0x0000000000007b50 0x000008 0x000008 R   0x1
# PHDR-NEXT: GNU_RELRO      {{.*}} 0x000000000000b6b8 0x000000000000b6b8 0x000008 0x000008 R   0x1
# PHDR-NEXT: GNU_RELRO      {{.*}} 0x00000000000124e8 0x00000000000124e8 0x0000d0 0x000b18 R   0x1
