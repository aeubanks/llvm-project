...
public:
  ...
  NodeMemRange makeMemRangeFromNonMem(SBInstruction *Top, SBInstruction *BotN) {
...
private:
  ...
  /// during IR revert().
  DenseMap<std::pair<SBInstruction *, SBInstruction *>, bool> DepCache;
  std::pair<SBInstruction *, SBInstruction *> getDepCacheKey(SBInstruction *I1,
                                                             SBInstruction *I2);
  void setDepCache(SBInstruction *I1, SBInstruction *I2, bool HasDep);
  bool getDepCache(SBInstruction *I1, SBInstruction *I2);
...
public:
  ...
  /// Instrs.
  size_t getProjectedApproxSize(const DmpVector<SBValue *> &Instrs);
  void setAAQueryBudget(int Budget) { AAQueryBudget = Budget; }
  bool hasDep(SBInstruction *SrcI, SBInstruction *DstI, int &AABudget);
  ...
  /// below.
  InstrInterval extendInternal(const DmpVector<SBValue *> &Instrs);

public:
  DependencyGraph(SBContext &Ctxt, AliasAnalysis &AA, int AAQueryBudget = 10,
                  Scheduler *Sched = nullptr);
  ...
  /// will crash if we are attempting to extend in both directions, above and
  /// below.
  InstrInterval extend(const DmpVector<SBValue *> &Instrs);
  ...
  void notifyInsert(SBInstruction *I);
