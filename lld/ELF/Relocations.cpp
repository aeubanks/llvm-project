...
Defined *GotPartitionSection::addEntry(Symbol &sym, int64_t addend) {
  auto [it, inserted] = entryMap.try_emplace({&sym, addend}, nullptr);
  if (!inserted)
    return it->second;

  uint64_t off = numEntries++ * ctx.target->gotEntrySize;
  if (sym.isTls()) {
    // FIXME: remove main GOT entry.
  ...
  it->second = makeDefined(ctx, file, sym.getName(), STB_LOCAL, STV_DEFAULT,
                           STT_NOTYPE, off, /*size=*/0, this);
  return it->second;
}
