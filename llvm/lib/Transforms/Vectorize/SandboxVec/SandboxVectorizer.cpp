...
bool SandboxVectorizerPass::runImpl(Function &F) {
  ...
  SBVecContext Ctxt(F.getContext(), *AA);
  ...
  SBPassRegistry PassRegistry(*TTI);

  // === SBVec Pass registration ===
  auto *VectorizerPM = cast<SBRegionPassManager>(PassRegistry.registerPass(
      std::make_unique<VectorizeScalars>(Ctxt, *SE, *DL, *TTI)));
  auto *VectorizePackOperandsPass = PassRegistry.registerPass(
      std::make_unique<VectorizePackOperands>(Ctxt, *SE, *DL, *TTI));
  (void)VectorizePackOperandsPass;
  PassRegistry.registerPass(
      std::make_unique<CollectAndDumpSeeds>(Ctxt, *SE, *DL, *TTI));
  PassRegistry.registerPass(std::make_unique<DumpRegion>());
  auto *AcceptOrRevertPass =
      PassRegistry.registerPass(std::make_unique<AcceptOrRevert>(Ctxt));
  auto *PackReusePass =
      PassRegistry.registerPass(std::make_unique<PackReuse>());
  (void)PackReusePass;
  // === End of SBVec Pass registration ===

  SBPassManager *InitPM = nullptr;
  if (UserDefinedPassPipeline != DefaultPipelineMagicStr) {
    ...
    auto *BBPM = cast<SBBBPassManager>(
        PassRegistry.registerPass(std::make_unique<SBBBPassManager>()));
