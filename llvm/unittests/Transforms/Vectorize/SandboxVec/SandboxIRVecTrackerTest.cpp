...
define void @foo(i32 %arg0, <2 x i32> %arg1, i32 %arg2) {
  ...
  DmpVector<SBValue *> Vals{Arg0, Arg1, Arg2};
  auto WhereIt = SBVecUtils::getInsertPointAfter(Vals, SBBB);
  auto *Pack = SBPackInstruction::create(Vals, WhereIt, SBBB, Ctxt);
  (void)Pack;
...
define void @foo(ptr %ptr, i32 %v0, i32 %v1) {
  ...
  auto *SBAdd0 = &*It++;
  auto *SBAdd1 = &*It++;
  auto *SBPack = cast<SBPackInstruction>(&*It++);
  SBAdd0->replaceUsesWithIf(SBAdd1,
...
define void @foo(ptr %ptr, <2 x i32> %v0) {
  ...
  auto *Add0 = &*It++;
  auto *Ret = &*It++;
  auto *New = cast<SBInstruction>(SBBinaryOperator::create(
      SBInstruction::Opcode::Add, Add0, Add0, Ret, Ctxt, "New"));
  EXPECT_EQ(Ctxt.getTracker().size(), 1u);
#ifndef NDEBUG
  unsigned Idx = 0;
  EXPECT_TRUE(isa<CreateAndInsertInstr>(Ctxt.getTracker().getChange(Idx++)));
...
define void @foo(i32 %v1, ptr %ptr) {
  ...
  const InstrInterval &View = Sched->getDAG().getView();
  const InstrInterval &DAGInterval = Sched->getDAG().getDAGInterval();
