...
/// schedule list.
static bool inList(SBInstruction *SBI, const Scheduler &Sched) {
  if (Sched.scheduleEmpty())
    return false;
  const DependencyGraph::Node *N = Sched.getDAG().getNode(SBI);
...
static SchedBundle *areBundle(SBInstruction *I1, SBInstruction *I2,
...
static SchedBundle *areBundle(Ts... Instrs, SBInstruction *ILast,
                              const Scheduler &Sched,
                              uint64_t CarryBundleSize = 0) {
  SchedBundle *InstrsBundle = areBundle(Instrs..., Sched, CarryBundleSize + 1);
...
static bool isScheduledBefore(SBInstruction *I1, SBInstruction *I2,
                              Scheduler &Sched) {
  auto *N1 = Sched.getDAG().getNode(I1);
  auto *N2 = Sched.getDAG().getNode(I2);
...
static uint64_t getNumScheduledNodes(const Scheduler &Sched) {
  uint64_t Cnt = 0;
  for (SchedBundle *SB = Sched.getTop(); SB != nullptr; SB = SB->getNext())
    Cnt += SB->size();
  return Cnt;
}
...
public:
  explicit BBIRChecker(SBBasicBlock *SBBB) : SBBB(SBBB), OS(Str) {
    OS << *SBBB;
  }
...
bb2:
  ...
  auto *BB1 = Ctxt.createSBBasicBlock(getBasicBlockByName(F, "bb1"));
  It = BB1->begin();
  auto *Phi0 = &*It++;
  auto *Add1 = &*It++;
  (void)Add1;
  ...
  auto *Br1 = &*It++;
  {
    ...
    Scheduler &Sched = *Ctxt.getScheduler(BB1);
    EXPECT_DEATH(Sched.trySchedule({Add0}), ".*different BB.*");
#endif
  }
  {
    Scheduler &Sched = *Ctxt.getScheduler(BB0);
...

  {
    ...
    EXPECT_EQ(&BB1->front(), Phi0);
    EXPECT_EQ(&BB1->back(), Br1);
    Sched.accept();
  }
}
...
lpad:
  ...
  (void)SBF;
  auto *LpadBB = Ctxt.getSBBasicBlock(getBasicBlockByName(F, "lpad"));
  ...
  auto *Br = &*It++;
  (void)Br;
  Scheduler &Sched = *Ctxt.getScheduler(LpadBB);
...
define void @foo(ptr noalias %ptr) {
  ...
  DependencyGraph &DAG = Sched.getDAG();
  auto It = BB->begin();
  auto *LdA = &*It++;
  auto *LdB1 = &*It++;
  auto *LdB2 = &*It++;
  ...
  SchedBundle *B1 = Sched.getBundle(LdA);
  (void)B1;
  SchedBundle *B2 = Sched.getBundle(LdB1);
  SchedBundle *B3 = Sched.getBundle(LdC);
  (void)B3;
...
define void @foo(ptr %ptr) {
  ...
  Sched.revert();
  Sched.startScheduling();
  DAG.extend(DmpVector<SBValue *>{St, Ld});
  EXPECT_EQ(LdN->getNumUnscheduledSuccs(), 1u);
  EXPECT_EQ(StN->getNumUnscheduledSuccs(), 0u);
}
...
define void @foo(i8 %v, ptr %ptr0, ptr %ptr1, ptr %ptr2) {
  ...
  EXPECT_TRUE(Sched.trySchedule({St0}));
  auto *NewI =
      SBStoreInstruction::create(V, Ptr3, /*Align=*/std::nullopt, St1, Ctxt);
  (void)NewI;
  auto &DAG = Sched.getDAG();
...
define void @foo(i8 %v0, i8 %v1, i8 %v2, ptr %ptr) {
  ...
  Sched.startScheduling();
  auto &DAG = Sched.getDAG();
  SchedulerAttorney::extendRegionAndUpdateReadyList(Sched, {St0, St1});
  EXPECT_TRUE(Sched.trySchedule({St1}));
  auto *St0N = DAG.getNode(St0);
  EXPECT_TRUE(Sched.getReadyList().contains(St0N));
  auto *NewI =
      SBStoreInstruction::create(V2, Ptr, /*Align=*/std::nullopt, St0, Ctxt);
...
#endif
  auto It = BB->begin();
  SBInstruction *Ld0 = &*It++;
  SBInstruction *Ld1 = &*It++;
  SBInstruction *St0 = &*It++;
  SBInstruction *St1 = &*It++;
  Sched.startTracking(BB);
...
define void @foo(ptr noalias %ptr, ptr noalias %ptr2) {
  ...
  {
    // Cluster(end)
    SchedBundle B1({Ld0, Ld1, Ret}, Sched);
...
define void @foo(ptr noalias %ptr1, ptr noalias %ptr2, ptr noalias %ptr3,
  ...
  auto *NewSI =
      SBStoreInstruction::create(Vec, Ptr, /*Align=*/std::nullopt, Ld2, Ctxt);
  SchedBundle *SB_NewI = Sched.getBundle(NewSI);
  ...
  SchedBundle *SB_Ld0 = Sched.getBundle(Ld0);
  SchedBundle *SB_St1 = Sched.getBundle(St1);
  SchedBundle *SB_St2 = Sched.getBundle(St2);
  SchedBundle *SB_St3 = Sched.getBundle(St3);
  SchedBundle *SB_Ld1 = Sched.getBundle(Ld1);
  SchedBundle *SB_Ld2 = Sched.getBundle(Ld2);
  (void)SB_Ld2;
  EXPECT_EQ(SB_St1, SB_St2);
  SchedBundle *SB_St1St2 = SB_St1;
...
define void @foo(ptr noalias %ptr1, ptr noalias %ptr2, ptr noalias %ptr3,
  ...
  auto *NewI =
      SBStoreInstruction::create(Ld0, Ptr, /*Align=*/std::nullopt, Add0, Ctxt);
...
define void @foo(ptr %ptr) {
  ...
  DAG.extend(DmpVector<SBValue *>{&*BB->begin(), BB->getTerminator()});
  auto &ReadyList = const_cast<ReadyListContainer &>(Sched.getReadyList());
...
bb1:
  ...
  SBVecContext Ctxt(C, AA);
  auto *BB1 = Ctxt.createSBBasicBlock(getBasicBlockByName(F, "bb1"));
  auto &DAG = Ctxt.getScheduler(BB1)->getDAG();
  auto It = BB1->begin();
  auto *Phi0 = &*It++;
  auto *Phi1 = &*It++;
  auto *Phi2 = &*It++;
  ...
  DAG.extend(DmpVector<SBValue *>{&*BB1->begin(), BB1->getTerminator()});
  Scheduler &Sched = *Ctxt.getScheduler(BB1);
  auto &ReadyList = const_cast<ReadyListContainer &>(Sched.getReadyList());
...
define void @foo(ptr %ptr, <2 x i8> %vec) {
  ...
  SchedulerAttorney::extendRegionAndUpdateReadyList(
      Sched, DmpVector<SBValue *>{&*BB->begin(), St1});
  EXPECT_TRUE(Sched.trySchedule({St0, St1}));
  ...
  auto *Ptr = cast<SBStoreInstruction>(St0)->getPointerOperand();
  auto *NewSI =
      SBStoreInstruction::create(Vec, Ptr, /*Align=*/std::nullopt, Ret, Ctxt);
  SchedBundle *NewSB = Sched.getBundle(NewSI);
  (void)NewSB;
  // The newly added node should be marked as 'Scheduled'.
...
define void @foo(ptr %ptr, i8 %val) {
  ...
  const InstrInterval &View = Ctxt.getDAG(BB).getView();
  const InstrInterval &DAGInterval = Ctxt.getDAG(BB).getDAGInterval();
  auto *Sched = Ctxt.getScheduler(BB);
  for (auto &SBI : *BB)
...
define void @foo(ptr %ptr) {
  ...
  DmpVector<SBValue *> LdBndl({Ld1});
  EXPECT_EQ(SchedulerAttorney::getBndlSchedState(Sched, LdBndl),
            SchedulerAttorney::BndlSchedState::FullyScheduled);
  ...
  DmpVector<SBValue *> NewBndl({Ld1, Ld0});
  EXPECT_EQ(SchedulerAttorney::getBndlSchedState(Sched, NewBndl),
            SchedulerAttorney::BndlSchedState::PartiallyOrDifferentlyScheduled);
  // Check that trimming works as expected.
  SchedulerAttorney::trimSchedule(Sched, NewBndl);
...
define void @foo(ptr %ptr) {
  ...
  SchedulerAttorney::trimSchedule(Sched, {Ld0, Ld1});
  auto *TopSB = Sched.getTop();
...
define void @foo(ptr %ptr, i8 %v0, i8 %v1) {
  ...
  auto It = BB->begin();
  auto *Foo0 = &*It++;
  auto *St1 = cast<SBStoreInstruction>(&*It++);
  ...
  auto *St0 =
      SBStoreInstruction::create(ArgV0, Ptr, /*Align=*/std::nullopt, St1, Ctxt);
  auto *St0N = DAG.getNode(St0);
...
define void @foo(float %v0, ptr %ptr) {
  ...
  auto *St = &*It++;
  auto *Ret = &*It++;
  DAG.extend(DmpVector<SBValue *>{Add, BB->getTerminator()});
  ...
  auto *NewI = SBStoreInstruction::create(Add, Ptr, /*Align=*/std::nullopt,
...
define void @foo(float %v0, float %v1, ptr noalias %ptr0, ptr noalias %ptr1, float %vNew) {
  ...
  auto *St0 = &*It++;
  auto *St1 = &*It++;
  DAG.extend(DmpVector<SBValue *>{St0, BB->getTerminator()});
  ...
  auto *NewI = SBStoreInstruction::create(Val, Ptr, /*Align=*/std::nullopt,
                                          /*InsertBefore=*/St1, Ctxt);
  auto *NewN = DAG.getNode(NewI);
  EXPECT_TRUE(NewN != nullptr);
  EXPECT_TRUE(NewN->dependsOn(St0N));
...
define void @foo(ptr %ptr, i8 %v0, i8 %v1, i8 %v2) {
  ...
  DAG.extend(DmpVector<SBValue *>{St0, St2});
  auto *St0N = DAG.getNode(St0);
  auto *St1N = DAG.getNode(St1);
