...
bb:
  ...
  auto It = SBBB.begin();
  auto *I0 = cast<SBInstruction>(&*It++);
  auto *I1 = cast<SBInstruction>(&*It++);
  auto *I2 = cast<SBInstruction>(&*It++);
  auto *I3 = cast<SBInstruction>(&*It++);
  SeedContainer SC;
...
bb:
  ...
  auto *S1 = cast<SBStoreInstruction>(&*It++);
  auto *S2 = cast<SBStoreInstruction>(&*It++);
  auto *S3 = cast<SBStoreInstruction>(&*It++);
  auto *S4 = cast<SBStoreInstruction>(&*It++);
  MemSeedContainer MSC(DL, SE);
  // Check begin() end() when empty.
...
bb:
  ...
  auto *SBSt0 = Ctxt.getSBValue(St0);
  auto *SBSt1 = Ctxt.getSBValue(St1);
  auto *SBSt2 = Ctxt.getSBValue(St2);
  auto *SBSt3 = Ctxt.getSBValue(St3);

  auto StoreSeedsRange = SC.getStoreSeeds();
  SeedBundle &SB = *StoreSeedsRange.begin();
  EXPECT_TRUE(std::next(StoreSeedsRange.begin()) == StoreSeedsRange.end());
  EXPECT_EQ(SB, DmpVector<SBValue *>({SBSt0, SBSt1, SBSt2, SBSt3}));
}
...
bb:
  ...
  SeedBundle &SB = *StoreSeedsRange.begin();
  EXPECT_TRUE(std::next(StoreSeedsRange.begin()) == StoreSeedsRange.end());
  EXPECT_EQ(SB, DmpVector<SBValue *>({SBSt0, SBSt2}));
}
...
bb:
  ...
  SeedBundle &SB = *SC.getStoreSeeds().begin();
  EXPECT_TRUE(std::next(SC.getStoreSeeds().begin()) ==
              SC.getStoreSeeds().end());
  EXPECT_EQ(SB, DmpVector<SBValue *>({SBSt0, SBSt1, SBSt3}));
}
