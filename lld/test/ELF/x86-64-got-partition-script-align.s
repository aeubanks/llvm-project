...
# RUN: ld.lld -T %t/lds -pie --got-partition-threshold=0x100000 %t/test.o -o %t.exe -Map=%t.map
...
# DISASM-EMPTY:
# DISASM-NEXT: <_start>:
# DISASM-NEXT:   {{.*}}movq{{.*}}(%rip), %rax{{.*}}# 0x2008

# DISASM:      Disassembly of section .text.2:
# DISASM-EMPTY:
# DISASM-NEXT: <_start_2>:
# DISASM-NEXT:   {{.*}}leaq{{.*}}(%rip), %rax{{.*}}# 0x2000e0 <foo>
