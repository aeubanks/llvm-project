...
define void @foo(ptr %ptrA, ptr %ptrB) {
  ...
  auto *VecSt =
      cast<SBInstruction>(VecBundle(DmpVector<SBValue *>({St0, St1})));
  (void)VecSt;
  auto *VecLd =
      cast<SBInstruction>(VecBundle(DmpVector<SBValue *>({Ld0, Ld1})));
...
define void @foo(float %v) {
  ...
  ASSERT_TRUE(isa<SBCallInstruction>(Opq0));
  ASSERT_TRUE(isa<SBInsertElementInstruction>(Opq1));

  Scheduler &Sched = *Ctxt.getScheduler(BB);
  Sched.startTracking(BB);
  VectorizeFromSeeds Vectorizer(BB, Ctxt, SE, DL, TTI);
  SmallPtrSet<SBInstruction *, 4> EraseCandidates;
  DmpVector<SBValue *> Seeds{Opq0, Opq1};
  SBRegion Rgn(*BB, Ctxt, TTI);
...
define void @foo(ptr %ptrA0, ptr %ptrA1) {
  ...
  auto *Add0 = &*It++;
  auto *Add1 = &*It++;
  DmpVector<SBValue *> Bndl({Ld0, Ld1});
