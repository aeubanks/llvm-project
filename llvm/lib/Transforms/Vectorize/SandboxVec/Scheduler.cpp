...
SchedBundle::SchedBundle(const DmpVector<SBValue *> &SBInstrs, Scheduler &Sched)
    : Sched(&Sched) {
...
void SchedBundle::cluster(SBBasicBlock::iterator BeforeIt, SBBasicBlock *SBBB) {
...
SchedBundle *Scheduler::createBundle(const DmpVector<SBValue *> &SBInstrs) {
  ...
  assert(all_of(SBInstrs,
                [this](SBValue *SBV) {
                  return DAG.inView(DAG.getNode(cast<SBInstruction>(SBV)));
                }) &&
...
SmallVector<DependencyGraph::Node *> ReadyListContainer::getContents() const {
...
void Scheduler::notifyNewViewNodes(
...
Scheduler::Scheduler(SBBasicBlock &SBBB, AliasAnalysis &AA, SBVecContext &Ctxt)
    : DAG(Ctxt, AA, AAQueriesBudget, this), SBBB(SBBB), Ctxt(Ctxt) {
  // Make sure all datastructures are reset.
  clearState();

  // Get notified about IR Instruction deletion.
  RemoveInstrCB =
      Ctxt.registerRemoveInstrCallbackBB(SBBB, [this](SBInstruction *SBI) {
        if (SBI->getParent() == &this->SBBB)
          notifyRemove(SBI);
      });

  InsertInstrCB =
      Ctxt.registerInsertInstrCallbackBB(SBBB, [this](SBInstruction *SBI) {
        if (SBI->getParent() == &this->SBBB)
          notifyInsert(SBI);
      });
...
void Scheduler::decrDepsReadyCounter(DependencyGraph::Node *N) {
  if (!TopDown) {
    for (auto *PredN : N->preds()) {
      if (!DAG.inView(PredN))
        continue;
...
bool Scheduler::tryScheduleUntil(const DmpVector<SBValue *> &Instrs) {
...
#ifndef NDEBUG
void Scheduler::verifyDirection(const DmpVector<SBValue *> &SBInstrs, bool TopDown) {
...
Scheduler::getBndlSchedState(const DmpVector<SBValue *> &Instrs) const {
  ...
  if (FullyScheduled) {
    ...
    // re-schedule.
    SchedBundle *SB = DAG.getNode(cast<SBInstruction>(Instrs[0]))->getBundle();
...
void Scheduler::trimSchedule(const DmpVector<SBValue *> &Instrs) {
  SBInstruction *TopI = !TopDown ? TopSB->getTopI() : TopSB->getBotI();
  SBInstruction *LowestI =
      !TopDown ? SBVecUtils::getLowest(Instrs) : SBVecUtils::getHighest(Instrs);
#ifndef NDEBUG
...
bool Scheduler::extendRegionAndUpdateReadyList(const DmpVector<SBValue *> &Instrs) {
#ifndef NDEBUG
  for (auto *SBV : Instrs) {
...
bool Scheduler::trySchedule(const DmpVector<SBValue *> &Instrs, bool TopDown) {
  Scheduling = true;
  if (!DirectionSet) {
  ...
  if (!SBVecUtils::areInSameBB(Instrs))
    return false;
...
void Scheduler::notifyRemove(SBInstruction *SBI, bool CalledByDAG) {
  ...
  if (Scheduling && !Ctxt.getTracker().inRevert()) {
    if (!N->isScheduled()) {
      decrDepsReadyCounter(N);
    }
...
void Scheduler::notifyInsert(SBInstruction *I) {
  if (!DAG.trackingEnabled())
    return;
  ...
  if (TopSB == nullptr || (!TopDown ? I->comesBefore(TopSB->getTopI())
    ...
    auto IsInSchedule = [this](SBInstruction *I) {
