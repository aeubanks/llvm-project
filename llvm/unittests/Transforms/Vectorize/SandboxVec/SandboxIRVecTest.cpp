...
define void @foo(<2 x i32> %vec) {
  ...
  auto *SBArg0 = SBF.getArg(0);
  auto It = SBBB.begin();
  auto *Shuffle0 = cast<SBShuffleInstruction>(&*It++);
...
#endif
  auto It = SBBB.begin();
  auto *SBAdd0 = cast<SBBinaryOperator>(&*It++);
  auto *Pack = cast<SBPackInstruction>(&*It++);
  auto *SBAdd1 = cast<SBBinaryOperator>(&*It++);
  auto *SBRet = &*It++;
...
define <4 x float> @foo(float %arg0, <2 x float> %arg1, float %arg2, <2 x float> %arg3, <4 x float> %arg4) {
  ...
  auto *Pack = cast<SBPackInstruction>(&*SBIt++);
  auto *SBRet = &*SBIt++;
...
define void @foo(<2 x i32> %vec) {
  ...
  EXPECT_TRUE(isa<SBReturnInstruction>(&*It++));
}
...
define void @foo(i32 %v0, i32 %v1, i32 %v2, <2 x i32> %vec) {
  ...
  EXPECT_TRUE(isa<SBInsertElementInstruction>(&*It++));
...
#endif
  EXPECT_TRUE(
      none_of(SBBB, [](SBValue &N) { return isa<SBPackInstruction>(&N); }));
}
...
define void @foo(i32 %v0, i32 %v1, i32 %v2, <2 x i32> %vec, <2 x i32> %vec2) {
  ...
  auto *InsertOpq1 = &*It++;
  EXPECT_EQ(InsertOpq1->getOpcode(), SBInstruction::Opcode::Insert);
  auto *InsertOpq2 = &*It++;
  EXPECT_EQ(InsertOpq2->getOpcode(), SBInstruction::Opcode::Insert);

  auto *Unpack = &*It++;
  EXPECT_EQ(Unpack->getOpcode(), SBInstruction::Opcode::Unpack);
  auto *ExtractOpq1 = &*It++;
  EXPECT_EQ(ExtractOpq1->getOpcode(), SBInstruction::Opcode::Extract);

  auto *Shuffle = &*It++;
  EXPECT_EQ(Shuffle->getOpcode(), SBInstruction::Opcode::Shuffle);
  auto *ShuffleBlend = &*It++;
  EXPECT_EQ(ShuffleBlend->getOpcode(), SBInstruction::Opcode::ShuffleVec);

  auto *Call = &*It++;
  EXPECT_EQ(Call->getOpcode(), SBInstruction::Opcode::Call);
...
define void @foo(i8 %val1, i8 %val2) {
  ...
  DmpVector<SBValue *> Ops{ArgVal1, ArgVal2};
  auto *Pack1 =
      cast<SBPackInstruction>(SBPackInstruction::create(Ops, BB, Ctxt));
  EXPECT_EQ(&BB->back(), Pack1);

  auto *Pack2 = cast<SBPackInstruction>(
      SBPackInstruction::create(Ops, /*InsertBefore=*/Ret, Ctxt));
  EXPECT_EQ(Ret->getPrevNode(), Pack2);

  auto WhereIt = BB->end();
  auto *Pack3 = cast<SBPackInstruction>(
      SBPackInstruction::create(Ops, WhereIt, BB, Ctxt));
  EXPECT_EQ(&BB->back(), Pack3);
}
...
define void @foo(ptr %ptr, i8 %val1, i8 %val2) {
  ...
  DmpVector<SBValue *> Ops{ArgVal1, ArgVal2, SBConstVec};
  auto WhereIt = SBVecUtils::getInsertPointAfter(Ops, BB);
...
define void @foo(float %arg0, <2 x float> %arg1, float %arg2) {
  ...
  auto ExtInstrs =
      SBPackInstructionAttorney::getLLVMInstrsWithExternalOperands(Pack);
  EXPECT_EQ(ExtInstrs, SmallVector<Instruction *>({Ins3, Extr1, Extr0, Ins0}));
}

TEST_F(SandboxIRVecTest, PackGetIRInstrs_InReverseProgramOrder) {
  parseIR(C, R"IR(
define void @foo(float %arg0, <2 x float> %arg1) {
  ...
  auto It = SBBB->begin();
  auto *Pack = cast<SBPackInstruction>(&*It++);
  auto IRInstrs = SBPackInstructionAttorney::getLLVMInstrs(Pack);
...
define void @foo(i8 %v0, i8 %v1, i8 %v2, i8 %v3) {
  ...
  DmpVector<SBValue *> SBPackInstructions({SBF->getArg(2), SBF->getArg(3)});
  auto WhereIt = SBVecUtils::getInsertPointAfter(SBPackInstructions, &SBBB);
  auto *NewPack = cast<SBPackInstruction>(
      SBPackInstruction::create(SBPackInstructions, WhereIt, &SBBB, Ctxt));
...
define void @foo(ptr %ptr, i32 %v0, <2 x i32> %v1, i32 %v2, <3 x i32> %v3) {
  ...
  DmpVector<SBValue *> SBPackInstructions({SBArg0, SBArg1, SBArg2, SBArg3});
  auto WhereIt = SBVecUtils::getInsertPointAfter(SBPackInstructions, &SBBB);
  auto *Pack = cast<SBPackInstruction>(
      SBPackInstruction::create(SBPackInstructions, WhereIt, &SBBB, Ctxt));
...
define void @foo(ptr %ptr) {
  ...
  DmpVector<SBValue *> SBPackInstructions({SBC, SBL});
  auto WhereIt = SBVecUtils::getInsertPointAfter(SBPackInstructions, &SBBB);
  auto *Pack = cast<SBPackInstruction>(
      SBPackInstruction::create(SBPackInstructions, WhereIt, &SBBB, Ctxt));
...
define void @foo(<2 x i8> %vec) {
  ...
  auto *Unpack1 = cast<SBUnpackInstruction>(
      SBUnpackInstruction::create(Arg, 0, 1, BB, Ctxt));
  EXPECT_EQ(&BB->back(), Unpack1);

  auto *Pack2 = cast<SBUnpackInstruction>(
      SBUnpackInstruction::create(Arg, 0, 1, Ret, Ctxt));
  EXPECT_EQ(Ret->getPrevNode(), Pack2);

  auto WhereIt = BB->end();
  auto *Pack3 = cast<SBUnpackInstruction>(
      SBUnpackInstruction::create(Arg, 0, 1, WhereIt, BB, Ctxt));
  EXPECT_EQ(&BB->back(), Pack3);
}
...
define void @foo(<4 x i8> %v) {
  ...
  EXPECT_EQ(SBVecUtils::getNumLanes(Unpack2), 1u);
...
define void @foo(<4 x i8> %v) {
  ...
  auto It = BB->begin();
  auto *Op = &*It++;
  auto WhereIt = SBVecUtils::getInsertPointAfter({Op}, BB);
  auto *Unpack = cast<SBInstruction>(SBUnpackInstruction::create(
      Op, /*Lane=*/0, /*LanesToUnpack=*/1, WhereIt, BB, Ctxt));
...
define void @foo(<2 x i8> %vec) {
  ...
  auto *Ret = cast<SBReturnInstruction>(&*It++);
  auto *Arg = SBF->getArg(0);
  ShuffleMask Mask{1, 0};
  auto *Shuff1 = cast<SBShuffleInstruction>(
      SBShuffleInstruction::create(Arg, Mask, BB, Ctxt));
  EXPECT_EQ(&BB->back(), Shuff1);

  auto *Shuff2 = cast<SBShuffleInstruction>(
      SBShuffleInstruction::create(Arg, Mask, Ret, Ctxt));
  EXPECT_EQ(Ret->getPrevNode(), Shuff2);

  auto WhereIt = BB->end();
  auto *Shuff3 = cast<SBShuffleInstruction>(
      SBShuffleInstruction::create(Arg, Mask, WhereIt, BB, Ctxt));
  EXPECT_EQ(&BB->back(), Shuff3);
}
...
define void @foo(ptr %ptr) {
  ...
  unsigned BBSize = BB->size();
  {
    ...
    EXPECT_EQ(&*It++, SBLd);
    auto *Pack = cast<SBPackInstruction>(Ctxt.getSBValue(Extr0));
    EXPECT_EQ(&*It++, Pack);
