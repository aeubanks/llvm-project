...
static SmallVector<DmpVector<SBValue *>>
getOperandBundlesSafe(const RangeT Bndl) {
  SmallVector<DmpVector<SBValue *>> OpBndls;
  ...
  case SBInstruction::Opcode::Store:
    return {getOperandBundle(Bndl, 0), getOperandBundle(Bndl, 1)};
  case SBInstruction::Opcode::PHI:
  case SBInstruction::Opcode::Opaque:
  case SBInstruction::Opcode::Ret:
  case SBInstruction::Opcode::AddrSpaceCast:
  case SBInstruction::Opcode::Call:
  case SBInstruction::Opcode::GetElementPtr:
    llvm_unreachable("Unimplemented");
  case SBInstruction::Opcode::Insert:
  case SBInstruction::Opcode::Extract:
  case SBInstruction::Opcode::ShuffleVec:
...
getUsesBundlesPicky(const DmpVector<SBValue *> &Bndl) {
...
VectorizeFromSeeds::VectorizeFromSeeds(SBBasicBlock *SBBB, SBVecContext &Ctxt,
                                       ScalarEvolution &SE,
                                       const DataLayout &DL,
...
SBValue *
VectorizeFromSeeds::createVecInstruction(const DmpVector<SBValue *> &Bndl,
                                         const DmpVector<SBValue *> &Operands,
                                         SBContext &Ctxt) {
  assert(all_of(Bndl, [](auto *SBV) { return isa<SBInstruction>(SBV); }) &&
         "Expect SBInstructions");

  // This lambda creates the instruction but won't add DebugInfo.
  auto CreateVecInstructionImpl = [&Ctxt](
                                      const DmpVector<SBValue *> &Bndl,
                                      const DmpVector<SBValue *> &Operands) {
    Type *ScalarTy =
        SBVecUtils::getElementType(SBVecUtils::getExpectedType(Bndl[0]));
    auto *VecTy =
        SBVecUtils::getWideType(ScalarTy, SBVecUtils::getNumLanes(Bndl));
    // TODO: Use a SBBuilder to avoid calling different ::create() functions.
    auto [WhereBB, WhereIt] = SBVecUtils::getInsertPointAfterInstrs(Bndl);
    ...
    case SBInstruction::Opcode::BitCast: {
      ...
      if (WhereIt != WhereBB->end())
        SBV = SBCastInstruction::create(VecTy, Opcode, Operands[0], &*WhereIt,
                                        Ctxt, "VCast");
      else
    ...
    case SBInstruction::Opcode::Select: {
      SBValue *SBV;
      if (WhereIt != WhereBB->end())
        SBV = SBSelectInstruction::create(Operands[0], Operands[1], Operands[2],
                                          &*WhereIt, Ctxt, "Vec");
      else
        SBV = SBSelectInstruction::create(Operands[0], Operands[1], Operands[2],
                                          WhereBB, Ctxt, "Vec");
    ...
    case SBInstruction::Opcode::FNeg: {
      ...
      if (isa<SBUnaryOperator>(SBV)) {
        if (UOp0->isFPMath()) {
          assert(!SBVecUtils::differentMathFlags(Bndl) &&
    ...
    case SBInstruction::Opcode::Load: {
      ...
      if (WhereIt != WhereBB->end())
        SBV = SBLoadInstruction::create(VecTy, Ptr, Ld0->getAlign(), &*WhereIt,
                                        Ctxt, "VecL");
      else
        SBV = SBLoadInstruction::create(VecTy, Ptr, Ld0->getAlign(), WhereBB,
                                        Ctxt, "VecL");
      return SBV;
    }
    case SBInstruction::Opcode::Store: {
  ...
  if (auto *NewI = dyn_cast<SBInstruction>(NewV))
    SBVecUtilsPrivileged::propagateMetadata(NewI, Bndl);
  return NewV;
}

#ifndef NDEBUG
void VectorizeFromSeeds::verifyDAGAndSchedule(const DmpVector<SBValue *> &Bndl,
  ...
  auto It = find_if(Bndl, [](SBValue *SBV) { return isa<SBInstruction>(SBV); });
  if (It != Bndl.end()) {
...
bool VectorizeFromSeeds::maybeStopRecursionForDebugging(
    const DmpVector<SBValue *> &Bndl, AnalysisResult *&AnalysisRes) {
  ++RecCnt;
...
SBValue *VectorizeFromSeeds::vectorizeRec(
    const DmpVectorView<SBValue *> &Bndl, AnalysisResult *AnalysisRes,
  ...
  case ResultID::PerfectReuseVector: {
    ...
    if (Res->needsShuffle()) {
      ...
      if (TopDown) {
        auto WhereIt = SBVecUtils::getInsertPointAfter({VecOp}, SBBB);
        New =
            SBShuffleInstruction::create(VecOp, ShuffMask, WhereIt, SBBB, Ctxt);
        auto *ShuffleI = cast<SBInstruction>(New);
        auto *VecUserI = cast<SBInstruction>(CommonVal);
        VecUserI->setOperand(OperandNo, ShuffleI);
      } else {
        auto WhereIt = SBVecUtils::getInsertPointAfter({CommonVal}, SBBB);
        New = SBShuffleInstruction::create(CommonVal, ShuffMask, WhereIt, SBBB,
                                           Ctxt);
      }
    } else {
      New = CommonVal;
  ...
  case ResultID::UnpackAndPack: {
    ...
    DmpVector<SBValue *> PackVals(Bndl.size());
    for (auto [Lane, SBV] : enumerate(Bndl)) {
      if (auto *Vec = InstrMaps.getVectorForScalar(SBV)) {
        ...
        auto WhereIt = SBVecUtils::getInsertPointAfter({Vec}, SBBB);
        auto *Unpack = SBUnpackInstruction::create(
            Vec, OrigLane, SBVecUtils::getNumLanes(SBV), WhereIt, SBBB, Ctxt);
        PackVals.push_back(Unpack);
      } else {
        PackVals.push_back(SBV);
      }
    }
    auto WhereIt = SBVecUtils::getInsertPointAfter(PackVals, SBBB);
    auto *Pack = SBPackInstruction::create(PackVals, WhereIt, SBBB, Ctxt);
    return Pack;
  }
  case ResultID::Unpack: {
    auto *Res = cast<Unpack>(AnalysisRes);
    auto WhereIt = SBVecUtils::getInsertPointAfter({Res->getVecOp()}, SBBB);
    auto *Unpack = SBUnpackInstruction::create(
        Res->getVecOp(), Res->getFirstLaneToUnpack(),
        Res->getNumLanesToUnpack(), WhereIt, SBBB, Ctxt);
    return Unpack;
  }
  ...
  case ResultID::Pack: {
    auto WhereIt = SBVecUtils::getInsertPointAfter(Bndl, SBBB);
    auto *New = SBPackInstruction::create(Bndl, WhereIt, SBBB, Ctxt);
    return New;
  }
  case ResultID::SimpleWiden: {
    ...
    if (!TopDown) {
      SmallVector<DmpVector<SBValue *>> OperandBndls =
          getOperandBundlesSafe(Bndl);
      DmpVector<SBValue *> Operands(2);
      for (auto [OpIdx, OpBndl] : enumerate(OperandBndls)) {
        bool IsAddrOperand =
            IsLoadOrStore &&
            OpIdx == SBVecUtils::getAddrOperandIdx(cast<SBInstruction>(Bndl0));
        auto *AnalysisRes =
      ...
      for (auto [OpIdx, OpBndl] : enumerate(getOperandBundles(Bndl))) {
        ...
        if (!Incomplete && Set.size() == 1) {
          ...
          auto *Pack = SBPackInstruction::create(
              OpBndl, /*BeforeI=*/SBVecUtils::getHighest(Bndl), Ctxt);
      ...
      if (Fail) {
        ...
        for (auto &Uses : UsesBndls) {
          DmpVector<SBValue *> Users(Uses.size());
          for (SBUse &Use : Uses)
...
SBInstruction *VectorizeFromSeeds::tryVectorize(
    const DmpVectorView<SBValue *> &Seeds, SBRegion &Rgn,
    SmallPtrSet<SBInstruction *, 4> &EraseCandidates, bool TopDown) {
  ...
  Ctxt.getScheduler(SBBB)->startFresh(SBBB);
#if !defined NDEBUG && defined SBVEC_EXPENSIVE_CHECKS
  Ctxt.getScheduler(SBBB)->getDAG().verify();
#endif
  auto *AnalysisRes =
...
void VectorizeFromSeeds::emitUnpacks(SBValue *Vec,
                                     const DmpVector<SBValue *> &Scalars) {
  assert(SBVecUtils::isVector(Vec) && "Expect vector");
  ...
  for (SBValue *OrigElem : Scalars) {
    ...
    Lane += SBVecUtils::getNumLanes(OrigElem);
  }
  ...
  for (auto [Lane, OrigElem] : UnpackData) {
    auto WhereIt = SBVecUtils::getInsertPointAfter({Vec}, SBBB);
    auto *Unpack = SBUnpackInstruction::create(
        Vec, Lane, SBVecUtils::getNumLanes(OrigElem), WhereIt, SBBB, Ctxt);
    ...
    if (RequiresBitcast) {
      ...
      SBInstruction *BeforeI = isa<SBInstruction>(Unpack)
                                   ? cast<SBInstruction>(Unpack)->getNextNode()
                                   : &*SBBB->getFirstNonPHIIt();
      Unpack = SBCastInstruction::create(OrigTy, SBInstruction::Opcode::BitCast,
                                         Unpack, BeforeI, Ctxt);
    }
    ...
    OrigElem->replaceUsesWithIf(Unpack, [this](auto *DstU, unsigned OpIdx) {
      return DstU == nullptr ||
             InstrMaps.getVectorForScalar(cast<SBInstruction>(DstU)) == nullptr;
    });
  }
}
...
bool VectorizeScalars::runOnSBBasicBlock(SBBasicBlock &BB) {
  ...
  auto RunOnSeed = [this, VecRegBits, &BB](SeedBundle &Seed,
    ...
    unsigned TyBits = DL.getTypeSizeInBits(SBVecUtils::getElementType(
        SBVecUtils::getExpectedType(Seed[Seed.getFirstUnusedElementIdx()])));
    ...
    for (unsigned SliceBits = std::min(VecRegBits, Seed.getNumUnusedBits(DL)),
                  E = 2 * TyBits;
         SliceBits >= E;
         SliceBits = SBVecUtils::getFloorPowerOf2(SliceBits / 2)) {
      if (Seed.allUsed())
        break;
      ...
      for (unsigned Offset = Seed.getFirstUnusedElementIdx(), OE = Seed.size();
        ...
        if (Seed.allUsed())
          break;
        DmpVectorView<SBValue *> SeedSlice =
