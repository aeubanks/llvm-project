...
public:
  AnalysisResult(ResultID ID, const DmpVector<SBValue *> &Vals,
                 SBAnalysis &Analysis);
...
public:
  ...
  PerfectReuseVector(SBValue *CommonOp, const DmpVector<SBValue *> &Vals,
                     SBAnalysis &A);
  PerfectReuseVector(SBValue *CommonOp, const ShuffleMask &ShuffMask,
                     const DmpVector<SBValue *> &Vals, SBAnalysis &A)
  ...
  PerfectReuseVector(SBValue *CommonOp, ShuffleMask &&ShuffMask,
                     const DmpVector<SBValue *> &Vals, SBAnalysis &Analysis)
...
public:
  UnpackTopDown(NoVecReason Reason, const DmpVector<SBValue *> &Vals,
                SBAnalysis &Analysis)
...
public:
  UnpackAndPack(const DmpVector<SBValue *> &Vals, SBAnalysis &Analysis)
...
public:
  Pack(NoVecReason Reason, const DmpVector<SBValue *> &Vals,
       SBAnalysis &Analysis)
...
public:
  SimpleWiden(const DmpVector<SBValue *> &Vals, SBAnalysis &Analysis)
...
public:
  KeepLane0(const DmpVector<SBValue *> &Vals, SBAnalysis &Analysis)
...
#ifndef NDEBUG
  /// Used for debugging.
  SmallVector<std::pair<DmpVector<SBValue *>, AnalysisResult *>>
      BndlAnalysisVec;
#endif

  void updateMap(const DmpVector<SBValue *> &Vals, AnalysisResult *Result);
  // AnalysisResult constructor calls updateMap().
  friend AnalysisResult::AnalysisResult(ResultID ID,
                                        const DmpVector<SBValue *> &Vals,
                                        SBAnalysis &Analysis);
...
public:
  ...
  cantVectorizeBasedOnOpcodesAndTypes(const DmpVector<SBValue *> &SBBndl);

  /// \Returns instructions that are probably dead in \p ProbablyDead.
  AnalysisResult *getBndlAnalysis(const DmpVector<SBValue *> &SBBndl,
