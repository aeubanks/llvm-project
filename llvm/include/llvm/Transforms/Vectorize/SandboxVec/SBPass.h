...
//
//
#ifndef LLVM_TRANSFORMS_VECTORIZE_SANDBOXVEC_SBPASS_H
#define LLVM_TRANSFORMS_VECTORIZE_SANDBOXVEC_SBPASS_H
...
public:
  ...
  virtual bool runOnSBFunction(SBFunction &SBF) = 0;
};
...
public:
  ...
  virtual bool runOnSBBasicBlock(SBBasicBlock &SBBB) = 0;
};

/// A pass that runs on a specified SB region, starting from a root node.
class SBRegionPass : public SBPass {
protected:
  SBRegionPass(const std::string &Name, const std::string &Flag,
                ClassID PassID)
      : SBPass(Name, Flag, PassID) {}

public:
  SBRegionPass(const std::string &Name, const std::string &Flag)
      : SBRegionPass(Name, Flag, ClassID::RegionPass) {}
  /// For isa/dyn_cast etc.
  static bool classof(const SBPass *From) {
    switch (From->getSubclassID()) {
    case ClassID::FnPass:
  ...
  virtual bool runOnRegion(SBRegion &Rgn) = 0;
};
} // namespace llvm
#endif // LLVM_TRANSFORMS_VECTORIZE_SANDBOXVEC_SBPASS_H
