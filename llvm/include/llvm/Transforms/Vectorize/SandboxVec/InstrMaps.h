...
static SBInstruction *pairFirst(const std::pair<SBValue *, SBValue *> &Pair) {
...
class InstructionMaps {
  ...
  DenseMap<SBValue *, DmpVector<SBValue *>> VectorToScalarsMap;
...
public:
  ...
  void registerVector(SBValue *Vec, const DmpVector<SBValue *> &Scalars);
  ...
  const DmpVector<SBValue *> &getScalarsFor(SBValue *Vec) const;
  ...
  getVectorsThatCombinedScalars(const DmpVector<SBValue *> &Bndl) const;
  std::optional<ShuffleMask>
  getShuffleMask(SBValue *Vec, const DmpVector<SBValue *> &OtherBndl) const;
