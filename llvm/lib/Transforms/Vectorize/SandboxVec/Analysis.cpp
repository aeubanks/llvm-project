...
AnalysisResult::AnalysisResult(ResultID ID, const DmpVector<SBValue *> &Vals,
                               SBAnalysis &Analysis)
    : SubclassID(ID), Analysis(Analysis) {
  Analysis.updateMap(Vals, this);
}

void SBAnalysis::updateMap(const DmpVector<SBValue *> &Vals, AnalysisResult *Result) {
  for (auto *SBV : Vals)
...
PerfectReuseVector::PerfectReuseVector(SBValue *CommonOp,
                                       const DmpVector<SBValue *> &Vals,
                                       SBAnalysis &A)
    : AnalysisResult(ResultID::PerfectReuseVector, Vals, A), CommonOp(CommonOp),
      ShuffMask(ShuffleMask::getIdentity(SBVecUtils::getNumLanes(CommonOp))) {}
...
SBAnalysis::cantVectorizeBasedOnOpcodesAndTypes(const DmpVector<SBValue *> &SBBndl) {
  ...
  if (SBVecUtils::getCommonScalarType(SBBndl) == nullptr)
  ...
  if (Opcode != SBInstruction::Opcode::Opaque && isa<SBInstruction>(TI0) &&
      ...
      SBVecUtils::differentMathFlags(SBBndl))
    return NoVecReason::DiffMathFlags;

  if (Opcode != SBInstruction::Opcode::Opaque && isa<SBInstruction>(TI0) &&
      SBVecUtils::differentWrapFlags(SBBndl))
  ...
  case SBInstruction::Opcode::BitCast: {
    ...
    Type *FromTy0 = SBVecUtils::getExpectedType(TI0->getOperand(0));
    if (any_of(drop_begin(SBBndl), [FromTy0](SBValue *SBV) {
          return SBVecUtils::getExpectedType(
                     cast<SBUser>(SBV)->getOperand(0)) != FromTy0;
        }))
      return NoVecReason::DiffTypes;
    return std::nullopt;
  }
  ...
  case SBInstruction::Opcode::Load:
    if (SBVecUtils::areConsecutive<SBLoadInstruction>(SBBndl, SE, DL))
      return std::nullopt;
    return NoVecReason::NonConsecutive;
  case SBInstruction::Opcode::Store:
    if (SBVecUtils::areConsecutive<SBStoreInstruction>(SBBndl, SE, DL))
      return std::nullopt;
    return NoVecReason::NonConsecutive;
...
getShuffleMaskForUnpacks(const DmpVector<SBValue *> &Bndl) {
...
AnalysisResult *SBAnalysis::getBndlAnalysis(
    const DmpVector<SBValue *> &SBBndl, const InstructionMaps &InstrMaps,
  ...
  if (Matches.size() == 1 && !SetIsIncomplete) {
    ...
    assert(NeedsShuffleMaskOpt->size() ==
               SBVecUtils::getNumLanes(ExistingVec) &&
  ...
  if (any_of(SBBndl, [](auto *SBV) { return !isa<SBInstruction>(SBV); })) {
    LLVM_DEBUG(
...
#endif
  auto *Scheduler =
      static_cast<SBVecContext &>(SBBB->getContext()).getScheduler(SBBB);
  ...
  case SBInstruction::Opcode::Unpack: {
    ...
    if ((int)ShuffMaskOpt->size() ==
        SBVecUtils::getNumLanes(VecOp->getType())) {
      CollectProbablyDead();
  ...
  case SBInstruction::Opcode::Select: {
    ...
    if (SBVecUtils::getNumLanes(Sel->getOperand(0)) !=
        SBVecUtils::getNumLanes(Sel->getOperand(1)))
  ...
  case SBInstruction::Opcode::ICmp: {
    ...
    if (any_of(drop_begin(SBBndl), [Op0Ty](SBValue *SBV) {
          return cast<SBInstruction>(SBV)->getOperand(0)->getType() != Op0Ty;
        }))
