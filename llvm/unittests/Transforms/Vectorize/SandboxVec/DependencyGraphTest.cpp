...
static void
expect_deps(const DependencyGraph &DAG, SBInstruction *I,
            const SmallVector<SBInstruction *> &ExpectedPredInstrs) {
  auto *N = DAG.getNode(I);
  EXPECT_NE(N, nullptr);
  SmallVector<DependencyGraph::Node *> ExpectedPreds;
  ...
  SmallVector<DependencyGraph::Node *> Preds(N->preds().begin(),
                                             N->preds().end());
  bool IsPermutation =
...
define void @foo(float %v, ptr noalias %ptr1, ptr noalias %ptr2) {
  ...
  DenseSet<DependencyGraph::Node *> Preds;
  ...
  DenseSet<DependencyGraph::Node *> L1Succs;
  for (auto *SuccN : L1->succs())
    L1Succs.insert(SuccN);
  EXPECT_EQ(L1Succs.size(), 1u);
  EXPECT_TRUE(L1Succs.count(A));

  DenseSet<DependencyGraph::Node *> L2Succs;
...
define void @foo(float %v1, float %v2) {
  ...
  auto &DAG = Ctxt.getScheduler(BB)->getDAG();
  DAG.extend(DmpVector<SBValue *>{&*BB->begin(), BB->getTerminator()->getPrevNode()});
...
define void @foo(float %v, ptr noalias %ptr1, ptr noalias %ptr2) {
  ...
  auto *Ld2 = &*It++;
  auto *Add = &*It++;
  DAG.extend(DmpVector<SBValue *>{Ld1, Add});
...
define void @foo(float %v, ptr noalias %ptr1, ptr noalias %ptr2) {
  ...
  auto *St1 = &*It++;
  auto *St2 = &*It++;
  DAG.extend(DmpVector<SBValue *>{St1, St2});
...
define void @foo(float %v, ptr noalias %ptr1, ptr noalias %ptr2) {
  ...
  SBBasicBlock *BB = Ctxt.createSBBasicBlock(&*F.begin());
  auto &DAG = Ctxt.getScheduler(BB)->getDAG();
  DAG.extend(DmpVector<SBValue *>{&BB->front(), &BB->back()});
...
exit:
  ...
  auto *BB = Ctxt.createSBBasicBlock(getBasicBlockByName(F, "loop"));
  auto &DAG = Ctxt.getScheduler(BB)->getDAG();
  DAG.extend(DmpVector<SBValue *>{&*BB->begin(), BB->getTerminator()});
...
define void @foo(float %v, float %v0, ptr noalias %ptr1, ptr noalias %ptr2) {
  ...
  DependencyGraph DAG(Ctxt, AA, AABudget);
  std::string Str;
  raw_string_ostream SS(Str);
  ...
  DAG.extend(DmpVector<SBValue *>{Ld2, Ld2});
  expect_deps(DAG, Ld2, {});

  DAG.extend(DmpVector<SBValue *>{Ld1, Ld1});
  expect_deps(DAG, Ld1, {});
  expect_deps(DAG, Ld2, {});

  DAG.extend(DmpVector<SBValue *>{St1, St1});
  expect_deps(DAG, Ld1, {});
  expect_deps(DAG, Ld2, {});
  expect_deps(DAG, St1, {Ld1});

  DAG.extend(DmpVector<SBValue *>{St2, St2});
  expect_deps(DAG, Ld1, {});
  expect_deps(DAG, Ld2, {});
  expect_deps(DAG, St1, {Ld1});
  expect_deps(DAG, St2, {Ld2});

  DAG.extend(DmpVector<SBValue *>{St0, St0});
  expect_deps(DAG, St0, {});
...
define void @foo(float %v, float %v0, ptr noalias %ptr1, ptr noalias %ptr2) {
  ...
  DependencyGraph DAG2(Ctxt, AA);
  DAG2.extend(DmpVector<SBValue *>{St0, St1});
  expect_deps(DAG2, St0, {});
  expect_deps(DAG2, Ld1, {St0});
  expect_deps(DAG2, Ld2, {});
  expect_deps(DAG2, St1, {St0, Ld1});

  DAG2.extend(DmpVector<SBValue *>{Ld1, St2});
  expect_deps(DAG2, St0, {});
  expect_deps(DAG2, Ld1, {St0});
  expect_deps(DAG2, Ld2, {});
  ...
  DependencyGraph DAG3(Ctxt, AA);
  DAG3.extend(DmpVector<SBValue *>{Ld2, Ld2});
  expect_deps(DAG3, Ld2, {});

  DAG3.extend(DmpVector<SBValue *>{St0});
  DAG3.extend(DmpVector<SBValue *>{St2});
  ...
  DependencyGraph DAG4(Ctxt, AA);
  DAG4.extend(DmpVector<SBValue *>{St0, St2});
  ...
  DAG4.extend(DmpVector<SBValue *>{Ld2, Ld2});
  expect_deps(DAG4, St0, {});
  expect_deps(DAG4, Ld1, {St0});
  expect_deps(DAG4, Ld2, {});
  expect_deps(DAG4, St1, {Ld1, St0});
...
define void @foo(float %v1, float %v2, ptr %ptr) {
  ...
  DAG.extend(DmpVector<SBValue *>{Ld1});
...
define void @foo(float %v1, float %v2, ptr %ptr) {
  ...
  DAG.extend(DmpVector<SBValue *>{St2});

  DAG.extend(DmpVector<SBValue *>{Ld1, DepLoad});
  auto *Ld1N = DAG.getNode(Ld1);
  auto *St1N = DAG.getNode(St1);
  auto *DepLoadN = DAG.getNode(DepLoad);
  auto *St2N = DAG.getNode(St2);
...
define void @foo(ptr %ptr) {
  ...
  Roots.clear();
  DAG.extend(DmpVector<SBValue *>{St3});
  Roots = DAG.getRoots();
  EXPECT_EQ(Roots.size(), 1u);
...
define void @foo(ptr %ptr) {
  ...
  {
    auto Rgn = DAG.extend(DmpVector<SBValue *>{St1});
    EXPECT_TRUE(Rgn == InstrInterval(St1, St1));
  }
  {
    auto Rgn = DAG.extend(DmpVector<SBValue *>{St0});
    EXPECT_TRUE(Rgn == InstrInterval(St0, St0));
  }
  {
    DAG.resetView();
    auto Rgn = DAG.extend(DmpVector<SBValue *>{Ld0, Ret});
    EXPECT_TRUE(Rgn == InstrInterval(Ld0, Ret));
  }
}
...
define void @foo(ptr noalias %ptr0, ptr noalias %ptr1) {
  ...
  DAG.extend(DmpVector<SBValue *>{Ld0, Ld1, St0, St1});
  DAG.resetView();
...
loop:
...

  {
    ...
    for (SBInstruction &I : drop_begin(*BB)) {
      auto *N = DAG.getNode(&I);
...

  {
    ...
    DAG.extend({Add0});
    DAG.extend({PHI});
    DAG.extend(DmpVector<SBValue *>{Ld0, BB->getTerminator()});
    auto *PHIN = DAG.getNode(PHI);
    for (SBInstruction &I : drop_begin(*BB)) {
      auto *N = DAG.getNode(&I);
      EXPECT_FALSE(dependencyPath(PHIN, N));
...
define void @foo(float %v1, float %v2, float %v3, <2 x float> %vec, ptr noalias %ptr1, ptr noalias %ptr2) {
  ...
  auto *Ptr = cast<SBStoreInstruction>(St2)->getPointerOperand();
  auto *NewI = SBStoreInstruction::create(Vec, Ptr, /*Align=*/std::nullopt,
                                          /*InsertBefore=*/Ret, Ctxt);
...
define void @foo(float %v1, float %v2, float %v3, ptr %ptr) {
  ...
  auto *Ptr = cast<SBStoreInstruction>(St1)->getPointerOperand();
  auto *St2 = SBStoreInstruction::create(SBF->getArg(1), Ptr, /*Align=*/
                                         std::nullopt,
                                         /*InsertBefore=*/St3, Ctxt);
...
define void @foo(float %v1, float %v2, float %v3, float %v4, <2 x float> %vec,
  ...
  auto *St4 = &*It++;
  auto *Ret = &*It++;
  DAG.extend(DmpVector<SBValue *>{St1, St4});
...
define void @foo(float %v1, float %v2, float %v3, float %new, ptr noalias %ptr1, ptr noalias %ptr2) {
  ...
  DAG.extend(DmpVector<SBValue *>{St1, St3});
  ...
  auto *NewI = SBStoreInstruction::create(Val, Ptr1, /*Align=*/std::nullopt,
                                          /*InsertBefore=*/St2, Ctxt);
...
define void @foo(float %v1, float %v2, float %v3, float %new, ptr noalias %ptr1, ptr noalias %ptr2) {
  ...
  DAG.resetView();
  EXPECT_TRUE(DAG.getView().empty());
  DAG.extend(DmpVector<SBValue *>{St2, St3});
  // Check the regions.
...
define void @foo(float %v1, float %v2, float %new, ptr noalias %ptr1, ptr noalias %ptr2) {
  ...
  auto *NewI = SBStoreInstruction::create(Val, Ptr2, /*Align=*/std::nullopt,
                                          /*InsertBefore=*/Ret, Ctxt);
...
define void @foo(ptr %ptr, i8 %B, i8 %A, i8 %N) {
  ...
  DAG.extend(DmpVector<SBValue *>{StB, StA});
...
define void @foo(ptr noalias %ptr, i8 %A) {
  ...
  DAG.extend(DmpVector<SBValue *>{LdB, StA});
...
define void @foo(ptr noalias %ptr0, ptr noalias %ptr1, i8 %v) {
  ...
  DAG.extend(DmpVector<SBValue *>{St0, Ret});
...
define void @foo(ptr %ptr, i8 %v1, i8 %v2, i8 %v3) {
  ...
  DAG.extend(DmpVector<SBValue *>{Add1, Ret});
  ...
  {
    SmallVector<DependencyGraph::Node *> Nodes;
    for (DependencyGraph::Node &N : reverse(DAG.makeRange(Add1N, St1N)))
      Nodes.push_back(&N);
    ASSERT_EQ(Nodes.size(), 2u);
  ...
  {
    SmallVector<DependencyGraph::Node *> Nodes;
    for (DependencyGraph::Node &N : drop_begin(DAG.makeRange(Add1N, St1N)))
      Nodes.push_back(&N);
    ASSERT_EQ(Nodes.size(), 1u);
    EXPECT_EQ(Nodes[0], St1N);
  }
  ...
  {
    SmallVector<DependencyGraph::Node *> Nodes;
    for (DependencyGraph::Node &N : DAG.makeMemRange(St1N, St1N))
      Nodes.push_back(&N);
    ASSERT_EQ(Nodes.size(), 1u);
    EXPECT_EQ(Nodes[0], St1N);
  }
  ...
  {
    SmallVector<DependencyGraph::Node *> Nodes;
    for (DependencyGraph::Node &N : DAG.makeMemRange(St1N, St3N))
      Nodes.push_back(&N);
  ...
  {
    SmallVector<DependencyGraph::Node *> Nodes;
    for (DependencyGraph::Node &N : DAG.makeMemRange(St1, St3))
      Nodes.push_back(&N);
    ASSERT_EQ(Nodes.size(), 3u);
    EXPECT_EQ(Nodes[0], St1N);
    EXPECT_EQ(Nodes[1], St2N);
  ...
  {
    SmallVector<DependencyGraph::Node *> Nodes;
    for (DependencyGraph::Node &N : reverse(DAG.makeMemRange(St1N, St3N)))
      Nodes.push_back(&N);
    ASSERT_EQ(Nodes.size(), 3u);
  ...
  {
    SmallVector<DependencyGraph::Node *> Nodes;
    for (DependencyGraph::Node &N : DAG.makeMemRangeFromNonMem(Add1, Add2))
      Nodes.push_back(&N);
    ASSERT_EQ(Nodes.size(), 2u);
    EXPECT_EQ(Nodes[0], St1N);
    EXPECT_EQ(Nodes[1], St2N);
  ...
  {
    SmallVector<DependencyGraph::Node *> Nodes;
    for (DependencyGraph::Node &N : DAG.makeMemRangeFromNonMem(Add2, Add3))
      Nodes.push_back(&N);
    ASSERT_EQ(Nodes.size(), 1u);
    EXPECT_EQ(Nodes[0], St3N);
  ...
  {
    SmallVector<DependencyGraph::Node *> Nodes;
    for (DependencyGraph::Node &N : DAG.makeMemRangeFromNonMem(Add1, St2))
      Nodes.push_back(&N);
    ASSERT_EQ(Nodes.size(), 2u);
    EXPECT_EQ(Nodes[0], St1N);
    EXPECT_EQ(Nodes[1], St2N);
  ...
  {
    SmallVector<DependencyGraph::Node *> Nodes;
    for (DependencyGraph::Node &N : DAG.makeMemRangeFromNonMem(St1, St2))
      Nodes.push_back(&N);
    ASSERT_EQ(Nodes.size(), 2u);
    EXPECT_EQ(Nodes[0], St1N);
    EXPECT_EQ(Nodes[1], St2N);
  ...
  {
    SmallVector<DependencyGraph::Node *> Nodes;
    for (DependencyGraph::Node &N : DAG.makeMemRangeFromNonMem(Add3, Ret))
      Nodes.push_back(&N);
  ...
  {
    SmallVector<DependencyGraph::Node *> Nodes;
    for (DependencyGraph::Node &N : DAG.makeMemRangeFromNonMem(Add3, Add3))
      Nodes.push_back(&N);
    ASSERT_EQ(Nodes.size(), 0u);
  }
