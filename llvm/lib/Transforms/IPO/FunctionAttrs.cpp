...
static bool inferInitializes(Argument &A, Function &F) {
  ...
  DenseMap<const BasicBlock *, std::pair<DenseSet<ArgumentUseOffset>, bool>>
      UsesPerBlock;
  ...
  for (const BasicBlock *BB : post_order(&F)) {
    ...
    if (UPB != UsesPerBlock.end() && !UPB->second.second) {
      continue;
    }
    ...
    if (UPB != UsesPerBlock.end()) {
      ...
      append_range(Uses, UPB->second.first);
